...
import io.hiis.service.core.api.Api.ApiError
import io.hiis.service.core.api.Api.ApiError.{ InternalServerError, MethodNotAllowed, NotFound }
...
trait ApiGatewayT extends Interceptors {
  self: Logging =>
  def routes: Seq[List[ServerEndpointT[Any, Any]]]

  def config: AppServerConfig

  private val allEndpoints: List[ServerEndpointT[Any, Any]] = routes.toList.flatten
  ...
  private val endUserSwaggerEndpoint =
    ...
    ).fromServerEndpoints(allEndpoints.filterNot(_.isExcluded), BuildInfo.name, BuildInfo.version)
  ...
  private val excludedEndUserSwaggerEndpoint =
    SwaggerInterpreter(
      ...
      .fromServerEndpoints(allEndpoints.filter(_.isExcluded), BuildInfo.name, BuildInfo.version)
  ...
  private val APP =
    ZioHttpInterpreter(
      ZioHttpServerOptions.customiseInterceptors
        ...
        .appendInterceptor(errorLoggingInterceptor)
        .appendInterceptor(cors) // Do not prepend this interceptor
        .options
    ).toHttp(
      allEndpoints
        ++ Seq(
...
trait Interceptors {
  ...
  /** Logs http errors */
  val errorLoggingInterceptor: RequestInterceptor[Task] =
  ...
  val requestLoggingInterceptor = RequestInterceptor.transformServerRequest[Task] {
    request: ServerRequest =>
      ...
      for {
        ...
        _ <-
          if (!excludedPaths.exists(url.startsWith)) {
            annotateWithRequest[Any, Nothing, Unit](
              logInfo(
