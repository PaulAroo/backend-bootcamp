...
trait Logging {
  ...
  private def loggerValue = SLF4J.logMarkerName(loggerName)
   ...
   *   unit
   */
  protected def logError(message: String, misc: Option[Json] = None)(implicit
...
trait SilentLogging extends Logging {
  override protected def logInfo(message: String, misc: Option[Json]): ZIO[Any, Nothing, Unit] =
    ZIO.unit

  override protected def logError(message: String, misc: Option[Json])(implicit
      cause: Throwable
  ): ZIO[Any, Nothing, Unit] = ZIO.unit
    ...
    def annotateWithRequest[R, E, T](
      ...
      request match {
        case SecuredRequest(_identity, _requestId, path, method, _, _sessionId) =>
          _sessionId match {
            case Some(value) =>
              zio @@ this.method(method) @@ this.path(path) @@ userId(
                _identity.id.value
              ) @@ requestId(
                _requestId.id
              ) @@ sessionId(value.id)
          ...
          zio @@ this.method(method) @@ this.path(path) @@ userId(_identity.id.value) @@ requestId(
            _requestId.id
          ) @@ sessionId(
        ...
        case UserAwareRequest(identity, requestId, path, method, _, sessionId) =>
          (identity, sessionId) match {
            case (Some(i), Some(s)) =>
              zio @@ this.method(method) @@ this.path(path) @@ this.userId(i.id.value) @@ this
                .requestId(
                  requestId.id
                ) @@ this
                .sessionId(s.id)
